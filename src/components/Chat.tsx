  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-transparent dark:bg-transparent rounded-md group-hover:bg-white dark:group-hover:bg-slate-900">
            TLDR, Ask me!
          </span>
      ...
      >
              ...
              <div className={`flex items-center ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {message.role === 'user' ? (
                  ...
                  <div className='text-xl'>
                    <Image src={profile} alt='Profile' width={30} height={30} className='rounded-full' />
                  </div>
                )}
              </div>
        ...
        <form onSubmit={(e) => e.preventDefault()} className="p-2 flex border-t border-gray-300 dark:border-gray-700">
          <textarea
            ref={textareaRef}
            className="flex-grow p-2 text-sm rounded mt-1 text-black dark:text-white resize-none overflow-hidden bg-white dark:bg-gray-800"
            value={input}
