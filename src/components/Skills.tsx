...
const Skills: React.FC<SkillsProps> = ({ skills }) => {
  ...
  return (
    <section
      ref={ref}
      id="skills"
      className="py-16 px-4 font-mono bg-[#1e1e1e] text-gray-200"
    >
      <div className="container mx-auto">
        ...
        <div className="mb-8 max-w-4xl mx-auto text-left bg-[#2d2d2d] p-4 rounded-md border-l-4 border-green-500 overflow-x-auto">
          <div className="flex items-center mb-2">
            ...
            <span className="text-lg font-semibold text-green-400">Skills.tsx</span>
          </div>
 ...
 */`}</code>
        ...
        <div className="max-w-4xl mx-auto mb-8 text-left">
          <div className="mb-4 flex items-center">
            ...
            <span className="text-xl font-semibold text-blue-400">Technical Expertise</span>
          </div>
          ...
          <div className="bg-[#2a2a2a] p-4 rounded-md border border-gray-700 mb-8">
            {/* Render skills as JSON-like structure */}
            {Object.entries(skillGroups).map(([category, categorySkills], groupIndex) => (
              <div key={category} className="mb-4 last:mb-0">
                <div className="mb-2">
                  <span className="text-green-400 pl-4">{category}</span>
                  <span className="text-gray-400">: </span>
                ...
                <div className="flex flex-wrap gap-2 pl-8 mb-2">
                  {categorySkills.map((skill, index) => (
                    <motion.div
                      key={index}
                      className="bg-[#1e1e1e] border border-gray-700 hover:border-blue-500 rounded-sm 
                               px-3 py-1 text-blue-300 cursor-default transition-all duration-200"
                    ...
                    >
                      ...
                      {skill}
                      <span className="text-gray-400">&quot;</span>
                      {index < categorySkills.length - 1 && <span className="text-gray-400">,</span>}
                    </motion.div>
                  ))}
                </div>
                ...
                <div className="pl-4">
                  <span className="text-purple-400">{"]"}</span>
                  {groupIndex < Object.keys(skillGroups).length - 1 && <span className="text-gray-400">,</span>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            <span className="text-orange-300">{"}"}</span><span className="text-gray-400">;</span>
          </div>
        </div>
      </div>
    </section>
