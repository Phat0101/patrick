...
const ExperienceItem: React.FC<ExperienceItemProps> = ({ experience, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="z-20 flex items-center justify-center bg-[#333333] shadow-lg w-8 h-8 rounded-full ml-[-13px] md:ml-0 relative">
        {iconMap[experience.icon]}
        <div className={`hidden md:block absolute top-1/2 transform -translate-y-1/2 ${index % 2 === 1 ? 'right-full mr-3 text-right' : 'left-full ml-3'} text-sm font-mono text-blue-400`}>
          {experience.date}
        </div>
      </div>
      <div className="bg-[#2d2d2d] border border-gray-700 hover:border-blue-500 rounded-md shadow-md w-full md:w-5/12 px-5 py-4 ml-4 md:ml-0 flex flex-col justify-between transition-all duration-200">
        <div className="mb-4 border-b border-gray-700 pb-2">
          <div className="flex items-center">
            <div className="text-yellow-400 mr-2">
              {iconMap[experience.icon]}
            </div>
            <h3 className="font-mono text-lg font-semibold text-blue-400">{experience.title}</h3>
          </div>
          <h4 className="font-mono text-sm text-gray-400 mt-1">@{experience.company.replace(/\s+/g, '_').toLowerCase()}</h4>
        ...
        <div className="line-numbers text-sm">
          ...
          <ul className="mt-2 text-gray-300 space-y-1">
            {experience.description.map((item, i) => (
              <li key={i} className="line pl-2 border-l border-gray-700 hover:border-green-500 hover:bg-[#3a3a3a] transition-colors">
                {item}
              </li>
            ))}
          </ul>
